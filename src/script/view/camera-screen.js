...
export function createModule(d) {
  ...
  const groundTop = d.WORLD_HEIGHT - d.GROUND_WORLD_HEIGHT;
  ...
  const maxCameraX = Math.max(0, d.WORLD_WIDTH - visibleWidth);
  const maxCameraY = Math.max(0, d.WORLD_HEIGHT - visibleHeight);
