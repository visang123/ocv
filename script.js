...
/** 25?®Í≥Ñ: 0 = ?¥Îß§ ?àÎÇ¥(2Ï¥?, 1 = ?®Ïïó ?¨Ïö© ?àÎÇ¥ */
let onboardingPostAppleSeedIntroPhase = 0;
let onboardingFruitIntroTimerId = null;
/** 16?®Í≥Ñ: 0 = Ï∂ïÌïò Î¨∏Íµ¨, 1 = ?¥Ïñ¥???àÎÇ¥(?Ä???§ÌÖù?Ä Í≥ÑÏÜç 16) */
let onboardingPostWaterCongratsPhase = 0;
/** 17?®Í≥Ñ: 0 = ?ùÎ¨ºÏßÄ???§Î™Ö 1, 1 = ?àÍ∞ú ?§Î™Ö 2 */
...
/** tutorial.html: ?ÖÏùò ?úÌÜ†Î¶¨Ïñº ?®Ïïó???¨ÎùºÏß????§Ïãú ?ìÍ∏∞ÍπåÏ?(ms) */
const TUTORIAL_MAIN_SEED_RESPAWN_MS = 5000;
let tutorialMainSeedRespawnTimerId = null;
/** ????Î¶¨Ïä§???¥ÌõÑ?êÎäî ?∏Î≤§¬∑?®Ïïó ?®Î≥¥???åÎëêÎ¶?Í∞ïÏ°∞ ?ùÎûµ(Ï≤??¨Ïù¥?¥Îßå). */
...
const guidePlaceholderHtml = "<p>?ÑÏßÅ ?¥Ïö©???ÜÏäµ?àÎã§!</p>";
...
/** ???®ÏúÑ Î°úÍ∑∏???†Ïõê(ovc-login.js?Ä ?ôÏùº ??. ?àÏúºÎ©?localStorageÎ≥¥Îã§ ?∞ÏÑ† */
...
/** ?úÎ≤Ñ Í≥ÑÏ†ï tutorial_done ??Î°úÏª¨ ?®Î≥¥???ÑÎ£å(?§Î•∏ Í∏∞Í∏∞¬∑?úÌÅ¨Î¶?Ï∞?Î≥µÏõê) */
...
/** ?îÎìú???∏Î≤§?êÎßå ?®Ïïó???àÏúºÎ©??îÎìú Î©îÏù∏ ?®Ïïó ?®Í∏∞(Î¶¨Î°ú????Ï§ëÎ≥µ Î∞©Ï?) */
...
/** ?îÎìú(index): Î°úÏª¨ ?Ä?? ?úÌÜ†Î¶¨Ïñº: ?¥Î≤à ?òÏù¥ÏßÄ Î∞©Î¨∏(session)Îß????îÎìú?Ä Î≥ÑÎèÑ. */
...
/** Í∞ÄÎ∞?UI ?¨Î°Ø??ÎπÑÎ©¥ Î∞îÎã• Ï±ÖÏùÑ Ï§çÍ∏∞ ?ÑÏóê 0(Í∞ÄÎ∞©Îßå Ï§çÏ? ÏßÅÌõÑ ?∏Î≤§??ÎπÑÏñ¥ ?àÏùå). */
...
/** ?ÑÏû¨ ?ùÎ¨ºÏßÄ???´ÏûêÎ•?500 ?àÍ∏à ?âÍ≥º ??Ï§ÑÏóê Î∞∞Ïπò(Í≤πÏπ® Î∞©Ï?) */
...
/** ?úÍ∞Å??ÎßëÏ? Íµ¨Ïó≠Î≥¥Îã§ ?¥Îèô ?ÅÏö©???ΩÍ∞Ñ ??≤å ÎßâÍ∏∞ ?ÑÏóê ÎßâÌûà???êÎÇå??Ï§ÑÏûÑ */
...
/** ?ùÎ¨ºÏßÄ??125+(?àÍ∞ú ?îÎìú 2)Î∂Ä??????E Ï§çÍ∏∞ ?àÏö© */
...
/** ?∞ÏÜç ?ÖÎ†•(?¥Î¶≠+Q ???ºÎ°ú ??Î≤àÏóê ?¨Îü¨ Ïπ∏Ïù¥ Ï∞®Îäî Í≤?Î∞©Ï? */
const PLANT_WATER_POUR_COOLDOWN_MS = 220;
let lastPlantWaterPourAt = 0;
/** Î¨?Ï§Ä ÏßÅÌõÑ updatePlantWaterLevel??Í∞ôÏ? ?ÑÎ†à?ÑÏóê Í∞êÏÜå?úÌÇ§ÏßÄ ?äÎèÑÎ°?*/
...
/** Î°úÏª¨¬∑Î©Ä??presence?????òÏßë ?ÅÌÉú ?úÏãú Í∏∏Ïù¥(?êÍ≤©?Ä rock_pickup Ï¢ÖÎ£å ??hold Î≥ÑÎèÑ) */
const WORLD_ROCK_PICKUP_ACTION_MS = 1000;
/** ?êÍ≤© ?åÎ†à?¥Ïñ¥: rock_pickup ?°ÏÖò???äÍ∏¥ ???ÅÌÉú ?çÏä§?∏Î? ?†Ï???ms(?§Î•∏ ?°ÏÖò?Ä REMOTE_ACTION_STATUS_HOLD_MS) */
...
/** ?êÌîÑ Ï§?player_stateÎ•????êÏ£º Î≥¥ÎÇ¥ ?êÍ≤© Ï∫êÎ¶≠??Í∂§Ï†Å???äÍ∏∞ÏßÄ ?äÍ≤å ??*/
...
/** ?ÑÏ≤¥ Ï±ÑÌåÖ ?ëÎëê(Î≥¥Ïù¥??Î™®Îëê?êÍ≤å ?úÏãú). */
const WORLD_CHAT_GLOBAL_PREFIX = "\uC804\uCCB4:";
/** ?ÑÍ∞Å ÏΩúÎ°†(U+FF1A) ??Î∞òÍ∞Å(U+003A)?ºÎ°ú ?µÏùº ??IME¬∑Î™®Î∞î?ºÏóê??Í∑ìÎßê/?ÑÏ≤¥ Íµ¨Î¨∏??Íπ®Ï?ÏßÄ ?äÍ≤å */
function normalizeWorldChatColonsForParsing(str) {
  return String(str || "").replace(/\uFF1A/g, ":");
}

/** UTF-16 ?úÎ°úÍ≤åÏù¥?∏Î? Ï§ëÍ∞Ñ?êÏÑú ?êÎ•¥ÏßÄ ?äÍ≤å ?êÎ¶Ñ */
...
/** ???ÑÌôò¬∑Î∞±Í∑∏?ºÏö¥??Î≥µÍ? ???åÎ†à?¥Ïñ¥????Í∞ÑÍ≤©??Í±¥ÎÑà?∞Î©¥ ?òÎπÑ Í≤ΩÎ°úÍ∞Ä ?úÍ∞Ñ?¥ÎèôÏ≤òÎüº Î≥¥ÏûÑ ???úÎ??àÏù∏ Î¶¨ÏÖã */
...
/** Í¥ÄÎ¶¨Ïûê ?úÎ≤Ñ: ?ùÎ¨ºÏßÄ???úÏãú¬∑?¥Ï†ú ?úÏóêÎß??§Ï†ú ?ùÎ¨º ?∞Ï∂ú(Î≥ÑÍ∞ú) */
...
/** world_state_conflict ÏßÅÌõÑ ÏßßÏ? ?Ä???†Ïòà ???∞ÏÜç Ï∂©Îèå¬∑?¥ÎßÅ Î£®ÌîÑ ?ÑÌôî */
...
/** ?∏ÏÖò ?äÍ∏∞Î©?ÎπÑÏñ¥ Í≥µÏú† Î¶¨ÏÖã?ºÎ°ú ?∏Ïãù ??appStorageKeysSharedWorldReset?ºÎ°ú Í∞ÄÎ∞??åÎûòÍ∑∏ÍπåÏßÄ ÏßÄ?åÏßê. local?Ä Í∏∞Ï°¥ */
...
/** ?êÍ≤© ?åÎ†à?¥Ïñ¥Í∞Ä Î©îÏù∏???ÑÎãå Ï∂îÍ? ?ëÎèô?¥Î? ?§Í≥† ?àÏùÑ ??(sessionId -> { isFull, bucketId }) */
...
playerBaseImage.src = "?¥Î?ÏßÄ/player-white.png";
...
/** index ?îÎìú + ?úÌÜ†Î¶¨Ïñº: ???®Ïïó?Ä worldLooseSeed¬∑seedCount ?ïÏ±Ö (src/game/groundSeed.js Ï∞∏Í≥†). */
...
/** ?îÎìú ?êÏä® ?®Ïïó Î™®Îìú: Ï§ëÎ≥µ id¬∑Í≥ºÎèÑ??seedCount ?ïÎ¶¨ */
...
/** ?òÎ¨¥ ?ÑÎ°ú ?¥Îèô Î≤îÏúÑÎ•??ìÌûê ???åÎ†à?¥Îãπ ÏµúÎ? Î≥Ä???úÍ∞Ñ?¥Îèô ?êÎÇå ?ÑÌôî) */
const TREE_DEPTH_CLAMP_MAX_STEP = 4;
/**
 * Î≥¥Ïù¥??ÎøåÎ¶¨Îß?= style.css .big-tree-roots (left 39, w 68, h 16, bottom -2).
 * ?ÑÏ≤¥ Í∏∞Îë• Î∞ïÏä§Î°?Í≤πÏπòÎ©???ÏßÄ?òÍ∞à ??Î™∏ÌÜµÎß?Í≤πÏ≥ê???òÎ¨¥ Î™®ÎìúÎ°??§Ïñ¥Í∞Ä ?úÍ∞Ñ?¥ÎèôÏ≤òÎüº Î≥¥ÏûÑ ??Î∞ú¬∑ÎøåÎ¶¨Îßå Í≤Ä??
 */
...
/** ?åÎ†à?¥Ïñ¥¬∑NPC Í≥µÌÜµ: Î®∏Î¶¨ ?óÏÑ†Í≥?ÎßêÌíç???¨Ïù¥(?îÎìú ?®ÏúÑ) */
const SPEECH_BUBBLE_GAP_ABOVE_HEAD_WORLD = 4;
/** NPC ÎßêÌíç?? Î®∏Î¶¨?Ä??Í∞ÑÍ≤©(?åÎ†à?¥Ïñ¥Î≥¥Îã§ Ï¢ÅÍ≤å) */
const NPC_SPEECH_BUBBLE_GAP_ABOVE_HEAD_WORLD = 0;
/**
 * npcY???§ÌîÑ?ºÏù¥??Î∞ïÏä§ ?óÏÑ†(?îÎìú yÍ∞Ä ?ëÏùÑ?òÎ°ù ?îÎ©¥ ??.
 * PNG ?ÅÎã® ?¨Î∞±ÎßåÌÅº ?ÑÎûòÎ°??¥Î†§ ?§Ï†ú Î®∏Î¶¨ Íº??Í∏?y.
 */
const NPC_HEAD_TOP_TRIM_WORLD = 8;
/** NPC ÎßêÌíç?? ?ëÏàò?ºÏàòÎ°?yÎ•?Ï§ÑÏó¨ Î®∏Î¶¨ Ï™??ÑÎûò)?ºÎ°ú ?¥Îèô */
const NPC_SPEECH_BUBBLE_SHIFT_DOWN_WORLD = 4;
/** ?∏Î≤Ñ ?¥Î¶Ñ ?ºÎ≤®Í≥??Ä???†ÎèÑ ÎßêÌíç???¨Ïù¥ Í∞ÑÍ≤©(?îÎ©¥ px) */
const WORLD_NPC_HOVER_NAME_GAP_ABOVE_BUBBLE_PX = 5;
/** ?åÎ†à?¥Ïñ¥ ÎßêÌíç?†Îßå: ?âÎÑ§??Î®∏Î¶¨ Í∑ºÏ≤ò) ?ÑÏóê ?ïÏã§???¨Î¶¨Í∏??îÎìú ?®ÏúÑ, ?¥ÏàòÎ°?ÎßêÌíç?????? */
...
/** ?êÍ≤© ?∏Ïõê??ÎßéÏùÑ?òÎ°ù ?¥ÎßÅ¬∑?Ä??Í∞ÑÍ≤©???ΩÍ∞Ñ ?òÎ†§ DB¬∑?¥Îùº?¥Ïñ∏??Î∂Ä?òÎ? Ï§ÑÏûÑ */
...
/** headTopWorldY: Ï∫êÎ¶≠??Î®∏Î¶¨(?óÏÑ†) ?îÎìú y. ÎßêÌíç??transform Í∏∞Ï? y(Î≤ÑÎ∏î Íº??Í∏? */
...
/** ?îÎìú(index) ?ÑÏö©: Í≥µÏú† ÎßµÎßå Ï¥àÍ∏∞?? ?úÌÜ†Î¶¨Ïñº Î™®Îìú¬∑tutorial.html ?ôÍ∏∞ ?ÜÏùå. */
...
document.addEventListener("keydown", function (event) {
  ...
  if (key === "q" && !event.repeat) {
    ...
    // Í±∞Îûò¬∑?úÏûë NPCÎ•??ùÎ¨º???¨Ïù∏Î≥¥Îã§ Î®ºÏ? Ï≤òÎ¶¨?òÍ≥†, Í∑ºÏ≤ò?êÏÑú Q????ÉÅ ?åÎπÑ(?§Î•∏ Q ?ôÏûë?ºÎ°ú ?òÏñ¥Í∞ÄÏßÄ ?äÏùå).
...
document.addEventListener("visibilitychange", function () {
  if (document.hidden) {
    ...
    // Î∞±Í∑∏?ºÏö¥??Î≥µÍ? ÏßÅÌõÑ ???ÑÎ†à?ÑÏóêÎß??∞Ïù¥??wallDeltaÍ∞Ä ÎπÑÏ†ï?ÅÏ†Å?ºÎ°ú Ïª§Ï?Î©?
...
/* Í∞ÄÎ∞?Î≤ÑÌäº(#world-bag-inventory) ?∏Î≤Ñ??document ?ÑÏûÑ?ºÎ°ú Ï≤òÎ¶¨ ???§ÌÅ¨Î¶ΩÌä∏ Ï£ºÏÑù */
if (plantHoverLabel) {
...
networkDebugButton.setAttribute("aria-label", "Î°úÍ∑∏");
...
adminDevPlantIndexPlusButton.setAttribute("aria-label", "?ùÎ¨ºÏßÄ??+100 (Í¥ÄÎ¶¨Ïûê)");
adminDevPlantIndexPlusButton.setAttribute("title", "?ùÎ¨ºÏßÄ??+100 (Í¥ÄÎ¶¨Ïûê)");
...
controlsButton.textContent = "Ï°∞ÏûëÎ≤?;
...
controlsOverlay.innerHTML =
  ...
  '<div class="controls-header"><strong>Ï°∞ÏûëÎ≤?/strong></div>' +
  '<div class="controls-list">' +
  '<div><span>W / \u2191</span><p>?ÑÎ°ú ?¥Îèô</p></div>' +
  '<div><span>A / \u2190</span><p>?ºÏ™Ω?ºÎ°ú ?¥Îèô</p></div>' +
  '<div><span>S / \u2193</span><p>?ÑÎûòÎ°??¥Îèô</p></div>' +
  '<div><span>D / \u2192</span><p>?§Î•∏Ï™ΩÏúºÎ°??¥Îèô</p></div>' +
  '<div><span>Space</span><p>?êÌîÑ</p></div>' +
  '<div><span>E</span><p>Ï§çÍ∏∞ / ?¥Î†§?ìÍ∏∞</p></div>' +
  '<div><span>Q</span><p>?¨Ïö© / ?Ä??/p></div>' +
  '<div><span>ÎßàÏö∞????/span><p>?ïÎ? / Ï∂ïÏÜå</p></div>' +
  '<div><span>Esc</span><p>?§Ï†ï ?¥Í∏∞ / ?´Í∏∞</p></div>' +
  '</div></div>';
...
function skipTutorialFromSettings() {
  if (!window.confirm("?úÌÜ†Î¶¨Ïñº??Í±¥ÎÑà?∞Í≥† ?êÏú†Î°?≤å ?åÎ†à?¥Ìï†ÍπåÏöî?")) {
    return;
  }
...
function replayTutorialFromSettings() {
  if (
    !window.confirm(
      "?úÌÜ†Î¶¨Ïñº??Ï≤òÏùåÎ∂Ä???§Ïãú ÏßÑÌñâ?†Íπå?? ?úÌÜ†Î¶¨Ïñº ?îÎ©¥?ºÎ°ú ?¥Îèô?©Îãà??"
    )
  ) {
    return;
  }
...
/** ?àÍ∞ú ÎßâÌûò ?êÏ†ï: Î∞úÎ≥¥??Î®∏Î¶¨(?ÑÏ™Ω)Í∞Ä ?àÍ∞ú???øÏùÑ ?åÍπåÏßÄ ?¥Îèô ?àÏö© */
...
/** rock-ground.svg(64√ó48) ??CSS `center bottom / contain` Í≥??ôÏùº?òÍ≤å ?àÌä∏Î∞ïÏä§ ?∞Ï∂ú */
const ROCK_GROUND_SVG_W = 64;
const ROCK_GROUND_SVG_H = 48;
/** viewBox ?ÅÎã® Îπ??¨Î∞± ?úÏô∏ ?????©Ïñ¥Î¶?∑Ïûî?îÎßå(?Ä??y 18~44) */
...
/** ry = ???îÏÜå ?óÏÑ†(?îÎìú). rockEl ?àÏúºÎ©??§Ï†ú ?åÎçî Î∞ïÏä§ Í∏∞Ï?(Ï§å¬∑CSS ?ºÏπò) */
...
/**
 * Î©îÏù∏ ?ëÎ¨º ÎØ∏Ïã¨?∏Îç∞ 'Ï§çÌûò'Îß??®Í≥† ?®Ïïó Ï∂îÏ†Å???ÜÏúºÎ©????®Ïïó?????òÏò¥ ???åÎûòÍ∑∏¬∑Í±¥Ï°??ÅÌÉú ?ïÎ¶¨
 */
function recoverWorldMainSeedIfOnboardingStuck() {
  ...
  // ÎßàÎ•∏ Î©îÏù∏ ?®Ïïó ?êÎèô ?úÍ±∞(updateSeedPosition)Í∞Ä pickedÎß?ÏºúÍ≥† ?§Ì??∞Î? ???®Í?.
  // ?¨Í∏∞???åÎûòÍ∑∏Î? ÏßÄ?∞Î©¥ ?®Ïïó???òÏÇ¥?ÑÎÇò ?Ä?¥Î®∏Í∞Ä ?ÅÏõê???ùÎÇòÏßÄ ?äÏùå.
...
/** Tutorial ?ÑÏö©: ??#seed ??extraSeeds ?§Ì??? ?îÎìú ?àÎ∏å?êÏÑú???∏Ï∂ú?òÎ©¥ ????(groundSeed.js). */
...
/** Î©îÏù∏ ?ëÎèô???Ö¬∑Ïö∞Î¨?????Ï¢åÌëú¬∑Ï∞?Îπ???Ï∂îÍ? ?ëÎèô?¥Î? ?§Í≥† ?àÏùÑ ?åÎäî Î≥ÑÎèÑ Î≥¥Í?Í∞??¨Ïö© */
...
/** ?êÏóê ???ëÎèô??Ï§ëÏã¨???∞Î¨º????Í∞ÄÍπåÏö¥???åÎ†à?¥Ïñ¥ Î∞úÎßå?ºÎ°ú??Í±∞Î¶¨ Î∞ñÏúºÎ°??òÍ???Í≤ΩÏö∞ */
...
/**
 * ?ëÎèô???¨Í∞Å?ïÍ≥º ?∞Î¨º ?¨Í∞Å?ïÏù¥ ?§Ï†úÎ°?Í≤πÏπò?îÏ?(?¨Ïú† px). Ï§ëÏã¨Í±∞Î¶¨Îß??∞Î©¥ ?ê¬∑ÌÜµ???∞Î¨º ?àÏ™Ω?ºÎ°ú ?§Ïñ¥Í∞?
 * ?ÑÎ†à?ÑÏóê?úÎèÑ 'Î©Ä??Í≥??òÏ? ?ºÍ∏∞ ?§Ìå® ??Í∞ôÏ? ?ÑÎ†à?ÑÏóê QÍ∞Ä ÎøåÎ¶¨Í∏?Ï∞??? Î∂ÑÍ∏∞Î°?Í∞Ä??Î¨∏Ï†úÍ∞Ä ?ùÍ?.
 */
...
/** ?úÌÅ¨Î¶?Ï∞???Î°úÏª¨?§ÌÜ†Î¶¨Ï?Í∞Ä ÎπÑÏñ¥???úÎ≤Ñ??tutorial_done???àÏúºÎ©?indexÎ°?Î≥¥ÎÇ¥Í∏??ÑÌï¥ Í≥ÑÏ†ï???ôÍ∏∞??*/
...
/**
 * ?úÌÜ†Î¶¨Ïñº????Î≤àÏù¥?ºÎèÑ ?ùÎÇ∏ Í≥ÑÏ†ï?Ä Î°úÏª¨???ÑÎ£å ?ÅÌÉúÍ∞Ä ?®ÏïÑ???úÎã§(Î¶¨Î°ú?ú¬∑ÌÉ≠ Ï¢ÖÎ£å¬∑Î°úÍ∑∏?ÑÏõÉ/Î°úÍ∑∏???ÑÏóê???îÎìú).
 * done ?åÎûòÍ∑∏Îßå ?†Ïã§??Í≤ΩÏö∞ ?Ä???§ÌÖù?ºÎ°ú Î≥µÍµ¨?úÎã§.
 * - "0": ?ïÏÉÅ ?ÑÎ£å/Í±¥ÎÑà?∞Í∏∞ ?±Ïùò Í∞?
 * - 34(ONBOARDING_MAX_STEP): Ï∂ïÌïò ?®Í≥Ñ ÏßÅÌõÑ¬∑7Ï¥??Ä?¥Î®∏Í∞Ä done???∞Í∏∞ ?ÑÏóê Î¶¨Î°ú?úÎêú Í≤ΩÏö∞
 */
...
/**
 * ??Î≤àÏù¥?ºÎèÑ ?îÎìú???§Ïñ¥??Í≥ÑÏ†ï: ?úÌÜ†Î¶¨Ïñº ?¨ÏÉù ?∏ÏÖò???ÑÎãàÎ©??úÌÜ†Î¶¨Ïñº ÎØ∏ÏôÑÎ£??ÅÌÉúÎ•?
 * Î≥∏Í≤å?ÑÏúºÎ°??òÎèåÎ¶∞Îã§(Î¶¨Î°ú?ú¬∑Î°úÍ∑∏ÏïÑ??Î°úÍ∑∏?∏¬∑ÌÉ≠ ?ÑÌôò ?ÑÏóê???îÎìú).
 */
...
/** index ?îÎìú + ?®Î≥¥???ÑÎ£å + ?úÎ≤Ñ ?ôÍ∏∞?? Î©îÏù∏ ?¨Î°Ø???¥Î? ?àÏúºÎ©??§Í≥† ?¨Í∏∞??extraPlantsÎ°úÎßå Ï∂îÍ? */
...
/** v1(ÏµúÎ? 27?®Í≥Ñ) ??v2(34?®Í≥Ñ) ?Ä???§ÌÖù ?¥Ï†Ñ */
...
function onboardingAllowsBucketQUse() {
  // 13: ?∞Î¨º?êÏÑú Í∏∑Í∏∞, 14~15: Î©îÏù∏ ?ëÎ¨º??Î∂ìÍ∏∞, 16+: ?òÎπÑ¬∑?§Ï†ï ???¥ÌõÑ?êÎèÑ ?ºÎ∞ò Î¨ºÏ£ºÍ∏?Í∞Ä??
  // ?¥Ï†Ñ?êÎäî 13¬∑15Îß??àÏö©??14?êÏÑú Î∂ìÍ∏∞/16 ?¥ÌõÑ QÍ∞Ä Î¨¥Î∞ò?ëÏ≤ò??Î≥¥Ïù¥??Î¨∏Ï†úÍ∞Ä ?àÏóà??
  ...
  switch (onboardingFlowStep) {
    case 1: {
      if (isNearWorldBagPickup() && !hasGuideBook) {
        movementTutorial.hideOverlay();
        setOnboardingCalloutVisible(true, "E?§Î? ?åÎü¨ Í∞ÄÎ∞©ÏùÑ ?åÏ??òÏÑ∏??");
    ...
    case 3: {
      if (guideOpen) {
        setOnboardingCalloutVisible(true, "?∏Î≤§?†Î¶¨(?Ä?•ÏÜå)Í∞Ä ?¥Î¶Ω?àÎã§.");
    ...
    case 4: {
      setOnboardingCalloutVisible(
        true,
        "spaceÎ∞îÎ? ?ÑÎ•¥Î©??êÌîÑÎ•??©Îãà?? ?¥Î≥¥?∏Ïöî!"
      );
      if (player) player.classList.add("onboarding-highlight");
      break;
    }
    case 5: {
      setOnboardingCalloutVisible(true, "?®Ïïó?ºÎ°ú ?¥Îèô?òÏÑ∏??");
      if (seed) seed.classList.add("onboarding-highlight");
      break;
    }
    case 6: {
      setOnboardingCalloutVisible(true, "e?§Î? ?åÎü¨ ?®Ïïó???åÏ??òÏÑ∏??");
      if (seed) seed.classList.add("onboarding-highlight");
      break;
    }
    case 7: {
    ...
    case 8: {
      setOnboardingCalloutVisible(true, "?ùÎ¨º???¨Ïù∏??Ï∞æÏïÑÍ∞Ä?∏Ïöî.");
    ...
    case 9: {
      ...
      setOnboardingCalloutVisible(true, "qÎ•??åÎü¨ ?ùÎ¨º???¨Ïù∏Í≥??Ä?îÌïò?∏Ïöî.");
      if (plantMaster) plantMaster.classList.add("onboarding-highlight");
      break;
    }
    case 10: {
      if (guideOpen) {
        const line1 = "?§Î™Ö??Ï∞∏Í≥†?òÏÑ∏??";
        const line2 = "esc ?êÎäî ?ÑÎ¨¥Í≥≥Ïù¥???¥Î¶≠???§Î™ÖÏ∞ΩÏùÑ ?´Ïúº?∏Ïöî.";
        setOnboardingCalloutVisible(
          true,
    ...
    case 11: {
      setOnboardingCalloutVisible(true, "?∞Î¨ºÍ∑ºÏ≤ò???ëÎèô?¥Î°ú ?¥Îèô?òÏÑ∏??");
    ...
    case 12: {
      setOnboardingCalloutVisible(
        true,
        "?ëÎèô??Í∑ºÏ≤òÎ°?Í∞Ä??E?§Î? ?åÎü¨ ?ëÎèô?¥Î? ?§Ïñ¥ Ï£ºÏÑ∏??"
      );
      if (bucket) bucket.classList.add("onboarding-highlight");
      break;
    }
    case 13: {
      setOnboardingCalloutVisible(
        true,
        "?∞Î¨ºÎ°??¥Îèô????Q?§Î? ?åÎü¨ Î¨ºÏùÑ Í∏∏Ïñ¥ Ï£ºÏÑ∏??"
      );
      if (well) well.classList.add("onboarding-highlight");
      if (bucket) bucket.classList.add("onboarding-highlight");
      break;
    ...
    case 14: {
      setOnboardingCalloutVisible(true, "Í∑∏Î?Î°??ÑÍπå ?¨Ï? ?®Ïïó?ºÎ°ú Í∞Ä?∏Ïöî.");
      if (plantSpot) plantSpot.classList.add("onboarding-highlight");
      break;
    }
    case 15: {
      setOnboardingCalloutVisible(true, "Q?§Î? ?åÎü¨ Î¨ºÏùÑ ÎøåÎ¶¨?∏Ïöî.");
      if (plantSpot) plantSpot.classList.add("onboarding-highlight");
      break;
    }
    case 16: {
      setOnboardingCalloutVisible(
        true,
        onboardingPostWaterCongratsPhase === 0
          ? "Ï∂ïÌïò?©Îãà?? ?ùÎ¨º ?§Ïö∞??Î≤ïÏùÑ Î∞∞Ïö∞?®Ïäµ?àÎã§."
          : "?ÑÏßÅ ?®Ïïò?µÎãà???ùÍπåÏßÄ ÏßÑÌñâ?¥Ï£º?∏Ïöî."
      );
      break;
    }
    case ONBOARDING_STEP_PLANT_INDEX: {
      ...
      setOnboardingCalloutVisible(
        true,
        onboardingPlantIndexIntroPhase === 0
          ? "?ùÎ¨ºÏßÄ?òÎäî ?ùÎ¨º???¨ÏúºÎ©??¨ÎùºÍ∞ëÎãà??"
          : "ÎßµÏùò ?àÍ∞úÍ∞Ä ?¥Ï†ú?òÎãà ???¨Î†§Î≥¥ÏÑ∏??"
      );
      break;
    }
    case ONBOARDING_STEP_DROP_BUCKET: {
      setOnboardingCalloutVisible(true, "E?§Î? ?åÎü¨ ?ëÎèô?¥Î? ?¥Î†§?ìÏúº?∏Ïöî.");
      if (bucket) bucket.classList.add("onboarding-highlight");
      if (player) player.classList.add("onboarding-highlight");
      break;
    }
    case ONBOARDING_STEP_CHAT: {
      setOnboardingCalloutVisible(
        true,
        "?í¨ Î≤ÑÌäº???åÎü¨ Ï±ÑÌåÖ???¥Í≥†, Î©îÏãúÏßÄÎ•??ÖÎ†•?????ÑÏÜ°??Î≥¥ÏÑ∏??"
      );
    ...
    case ONBOARDING_STEP_HEART: {
      setOnboardingCalloutVisible(
        true,
        "?§Ô∏è Î≤ÑÌäº???åÎü¨ ?òÌä∏Î•?Î≥¥ÎÇ¥ Î≥¥ÏÑ∏?? ?§Î•∏ ?åÎ†à?¥Ïñ¥?êÍ≤å Î∞òÏùë???ÑÌï† ???àÏñ¥??"
      );
    ...
    case ONBOARDING_STEP_SAD: {
      setOnboardingCalloutVisible(
        true,
        "?ò¢ Î≤ÑÌäº???åÎü¨ ?¨Ìçº?îÎ? Î≥¥ÎÇ¥ Î≥¥ÏÑ∏??"
      );
      if (worldSadBtn) worldSadBtn.classList.add("onboarding-highlight");
      break;
    }
    case ONBOARDING_STEP_ROCK: {
      setOnboardingCalloutVisible(true, "?ÖÏùò ?åÏóê Í∞ÄÍπåÏù¥ Í∞Ä??E?§Î°ú Ï§çÏïÑ Î≥¥ÏÑ∏??");
    ...
    case ONBOARDING_STEP_BUTTERFLY: {
      setOnboardingCalloutVisible(
        true,
        "?†ÏïÑ?§Îãà???òÎπÑ??Í∑ºÏ†ë?òÏó¨ E ?êÎäî QÎ°??°ÏïÑ Î≥¥ÏÑ∏??"
      );
      Object.keys(butterflyRenderById).forEach(function (id) {
        const entry = butterflyRenderById[id];
        if (entry && entry.element) {
    ...
    case ONBOARDING_STEP_TRADE_MASTER: {
      setOnboardingCalloutVisible(true, "Í±∞Îûò???¨Ïù∏?êÍ≤å Í∞Ä??Q?§Î°ú ?Ä?îÌï¥ Î≥¥ÏÑ∏??");
      if (tradeMaster) tradeMaster.classList.add("onboarding-highlight");
      break;
    }
    case ONBOARDING_STEP_ALCHEMY_MASTER: {
      setOnboardingCalloutVisible(true, "?∞Í∏à?†Ïùò ?¨Ïù∏?êÍ≤å Í∞Ä??Q?§Î°ú ?Ä?îÌï¥ Î≥¥ÏÑ∏??");
      if (alchemyMaster) alchemyMaster.classList.add("onboarding-highlight");
      break;
    }
    case ONBOARDING_STEP_ZOOM_INTRO: {
      setOnboardingCalloutVisible(true, "?§ÌÅ¨Î°§Ìï¥ ÎßµÏùÑ Ï∂ïÏÜå,?ïÎ? ?¥Î≥¥?∏Ïöî.");
      break;
    }
    case ONBOARDING_STEP_ZOOM_MIN: {
      setOnboardingCalloutVisible(true, "Í∞Ä???ëÍ≤å Ï∂ïÏÜå ?¥Î≥¥?∏Ïöî.");
      break;
    }
    case ONBOARDING_STEP_TREE_APPROACH: {
      setOnboardingCalloutVisible(true, "?ïÏ§ë?????òÎ¨¥Î°??¥Îèô?òÏÑ∏??");
    ...
    case ONBOARDING_STEP_TREE_CLIMB: {
      setOnboardingCalloutVisible(
        true,
        "?òÎ¨¥Î•??¥Îèô?òÏó¨ ?¨Îùº?ÄÍ≥??¥Îß§??Í∑ºÏ≤òÎ°??¥Îèô?òÏÑ∏??"
      );
      if (bigTree) bigTree.classList.add("onboarding-highlight");
    ...
    case ONBOARDING_STEP_PICK_APPLE: {
      setOnboardingCalloutVisible(true, "e?§Î? ?åÎü¨ ?¥Îß§Î•??∞ÏÑ∏??");
      highlightUnpickedApplesForTutorial();
      break;
    }
    case ONBOARDING_STEP_EAT_APPLE: {
      setOnboardingCalloutVisible(true, "Í∞ÄÎ∞©ÏùÑ ?????¨Í≥º Ïπ∏ÏùÑ ?åÎü¨ Î®πÏúº?∏Ïöî.");
      if (worldBagInventory) worldBagInventory.classList.add("onboarding-highlight");
      if (bagInventoryPanel) {
    ...
    case ONBOARDING_STEP_EXTRA_SEED: {
      const lineSeed = "?®Ïïó???ùÍ≤º?ºÎãà ?êÌïò??Í≥≥Ïóê ?¥Î¶≠???¨Ïö©?òÏÑ∏??";
      const lineB = "?òÎ¨¥Î∞ñÏúºÎ°??¥Îèô?òÏÑ∏??";
      if (onboardingPostAppleSeedIntroPhase === 0) {
        setOnboardingCalloutVisible(true, "?®Ïïó???ªÏóà?µÎãà??");
      } else {
        setOnboardingCalloutVisible(
          true,
    ...
    case ONBOARDING_STEP_SETTINGS_ESC: {
      setOnboardingCalloutVisible(
        true,
        "EscÎ•??åÎü¨ ?§Ï†ï?????? ?§Ïãú EscÎ°??´ÏïÑ Î≥¥ÏÑ∏??"
      );
      break;
    }
    case ONBOARDING_STEP_COMPLETE: {
      setOnboardingCalloutVisible(true, "Ï∂ïÌïò?©Îãà?? ?úÌÜ†Î¶¨Ïñº???ùÎÇ¨?µÎãà??!");
      break;
    }
    default:
      setOnboardingCalloutVisible(false, "");
...
function isPlantMasterVisible() {
  ...
  /** Î≥∏Ìé∏: Ï≤??Ä???ÑÏóê??Î©îÏù∏ ?ëÎ¨º ?¨Í∏∞ ?ÑÏóê??Ï∞æÏùÑ ???àÍ≤å */
...
/**
 * World hub: worldLooseSeed(?©ÏÑ± ?ÑÎ≥¥) + extraSeeds + ????+ ?ëÎèô??
 */
...
/**
 * Tutorial / ?®Î≥¥??Ï§?index: ??#seed(?úÌÜ† Î©îÏù∏) ???§Ì??? Í∑???extraSeeds, ?ëÎèô??
 */
...
/** Tutorial: extraSeedsÎß? World hub: Î≥¥Ïù¥???ôÏïà ?©ÏÑ± worldLoose ?ÑÎ≥¥Í∞Ä ?∞ÏÑ†(Í∞ôÏ? Ï¢åÌëú ?ïÏ±Ö?Ä groundSeed.js). */
...
function rebuildWorldExtraBucketDom() {
  ...
  appleState.worldExtraBuckets.forEach(function (entry) {
    ...
    el.src = entry.isFull ? "?¥Î?ÏßÄ/bucket-full.png" : "?¥Î?ÏßÄ/bucket-empty.png";
    el.alt = "";
    el.setAttribute("aria-hidden", "true");
    el.draggable = false;
    if (insertBeforeEl) {
...
function updateWorldExtraBuckets() {
  ...
  appleState.worldExtraBuckets.forEach(function (entry) {
    ...
    entry._el.src = entry.isFull ? "?¥Î?ÏßÄ/bucket-full.png" : "?¥Î?ÏßÄ/bucket-empty.png";
  });
}
...
/** Í±∞Îûò ÍµêÌôò: Í∏∞Ï°¥ ?ëÎèô?¥Îäî ?®Í∏∞Í≥?ÎßµÏóê ?ëÎèô?¥Î? ?òÎÇò ??Î∞∞Ïπò */
...
/** ?∞Î¨º ?®ÏãúÎ∏?Î¶¨ÌïÑ ?±Í≥º ÎßûÏ∂∞ ?§Î•∏ ?¥Îùº?Ä lastRefillAt ?¥ÏÑù???¥Í∏ã????ñ¥?∞Í∏∞Í∞Ä Î∞òÎ≥µ?òÎäî Í≤ÉÏùÑ Ï§ÑÏûÑ */
...
/**
 * Í≥µÏú† ?îÎìú ?Ä??ÏßÅÏ†Ñ: ?®ÏãúÎ∏?Î¨?Í∞êÏÜå¬∑?∞Î¨º Î¶¨ÌïÑ???ÑÏû¨ ?úÍ∞ÅÍπåÏ? Î∞òÏòÅ??
 * ?§Îûò???òÎ∂Ñ/?∞Î¨º Í∞íÏù¥ ?§ÎÉÖ?∑Ïóê ?§Î¶¨ÏßÄ ?äÍ≤å ??(Î©Ä?∞Ïóê??Í≤åÏù¥ÏßÄÍ∞Ä ?§Ï≠â?†Ï≠â???êÏù∏).
 */
...
function getSharedWorldSnapshot() {
  ...
  return {
    /** Î©Ä?? ?ùÎ¨º ?Ä?ÑÏä§?¨ÌîÑ?Ä Í∞ôÏ? ?úÍ∞ÅÏ∂ïÏúºÎ°??êÏñ¥ `rebasePlantModelTimestampsToLocalNow`??refTimeÍ≥?Í¥¥Î¶¨ Í∞êÏÜå */
    ...
    apples: {
      ...
      // ?îÎìú(?®Î≥¥???ÑÎ£å): ???®Ïïó?Ä worldLooseSeed 1?¨Î°ØÎß?Í≥µÏú†. ?¨Ï? ?®Ïïó(stub)Îß?extraSeeds.
...
function applySharedWorldSnapshot(snapshot, serverRowUpdatedAt) {
  ...
  if (
    ...
    ignoreSnapshotInventorySeedsUntil = Date.now() + 15000;
    persistOvcLastAppliedWorldResetToken(lastAppliedWorldResetToken);
    // Í≥µÏú† ?Í≥? Î°úÏª¨ ???Ï∫êÏãú ?? + Îßµ¬∑Ïûê??Í∏∞Î≥∏???†Î¶¨??????Î≥???????)
    clearStoredKeys(appStorageKeysSharedWorldReset);
    isWorldDirty = false;
    applyDefaultState({ sharedWorldResetOnly: true });
    savePlayerPosition(true);
...
/** ?∏ÏÖòÎß??∞Î©¥ ????óê??ÎπÑÏñ¥ Í≥µÏú† Î¶¨ÏÖã?ºÎ°ú ?§Ïù∏ ??appStorageKeysSharedWorldReset?ºÎ°ú Í∞ÄÎ∞??åÎûòÍ∑∏ÍπåÏßÄ ÏßÄ?åÏßê. localÍ≥??ôÍ∏∞?? */
...
/**
 * Î©Ä??Í≥µÏú† ?îÎìú: ?§ÎÉÖ?∑Ïùò `rebasePlantModelTimestampsToLocalNow`¬∑sanitizeÍ∞Ä
 * `getSynchronizedNow()`(?úÎ≤Ñ ???úÍ∞Å Í∏∞Ï? ?§ÌîÑ???ºÎ°ú ÎßûÏ∂∞???àÎäî??
 * Î¨?Í∞êÏÜåÎß?`Date.now()`Î•??∞Î©¥ ?§ÌîÑ?ãÎßå??Í≤ΩÍ≥ºÍ∞Ä Î≤åÏñ¥?????±Ïóê Î¨ºÏù¥ ??Îπ†Ï?Í≥?
 */
...
function syncWorldState(forceSave) {
  ...
  window.OVCOnline.saveWorldState(
    ...
    addNetworkDebugLog(
      "world save error: " + (error && error.message ? error.message : "?®Îùº???úÎ≤Ñ ?ïÏù∏ ?ÑÏöî")
    );
...
function saveSharedWorldAndReload(options) {
  ...
  saveResetSnapshot().then(function (row) {
    ...
    addNetworkDebugLog(
      "world reset save error: " + (error && error.message ? error.message : "?®Îùº???úÎ≤Ñ ?ïÏù∏ ?ÑÏöî")
    );
    showThrottledWorldSyncToast(
      "\uC6D4\uB4DC \uC800\uC7A5\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4. \uC7A0\uC2DC \uD6C4 \uB2E4\uC2DC \uC2DC\uB3C4\uD569\uB2C8\uB2E4."
    );
...
function pollWorldState(forcePoll) {
  ...
  window.OVCOnline.loadWorldState(
    ...
    addNetworkDebugLog(
      "world poll error: " + (error && error.message ? error.message : "?®Îùº???úÎ≤Ñ ?ïÏù∏ ?ÑÏöî")
    );
    showThrottledWorldSyncToast(
...
function updateSeedPosition() {
  ...
  // ?îÎìú ?êÏä® ??Î™®Îìú: ???®Ïïó?Ä worldLooseSeed ??Í∞úÎßå ???úÌÜ†??#seed???®Í?(?êÏóê ??Í≤ΩÏö∞Îß??úÏãú).
  ...
  seed.src = plantRuntime.isSeedDry ? "?¥Î?ÏßÄ/seed-dry.png" : "?¥Î?ÏßÄ/seed.png";
...
/**
 * ÏßÄ???????¨Î°Ø(WORLD_LOOSE_SEED_* = SEED_START)??Í≤πÏπú extraSeeds ???§ÌîÑ?ºÏù¥?∏Îäî ?®Í?.
 * ?úÌÜ† #seed¬∑?îÎìú ?êÏä® img?Ä legacy ?¨Î°Ø???ôÏãú??Î≥¥Ïù¥??Í≤ΩÏö∞ Î∞©Ï?.
 */
...
function updateExtraSeedsAndPlants() {
  ...
  if (usesWorldLooseSeedMode()) {
    ...
    if (!worldLooseSeedElement) {
      ...
      worldLooseSeedElement.src = "?¥Î?ÏßÄ/seed.png";
  ...
  appleState.extraSeeds.forEach(function (extraSeed) {
    ...
    extraSeed.element.src = isDry ? "?¥Î?ÏßÄ/seed-dry.png" : "?¥Î?ÏßÄ/seed.png";
...
/** ?Ä?•¬∑Ïä§?ÖÏÉ∑???±Ïû• msÍ∞Ä ?ÑÏû¨ constants?Ä ?§Î•¥Î©?ÎßûÏ∂§(?? Í∞ÄÎ£?2Î∂Ñ‚Üí3Ï¥àÎ°ú Î∞îÍæº ?§Ïóê????duration???®Îäî Í≤ΩÏö∞) */
...
/**
 * ?¨Ï? ÏßÅÌõÑ ?§ÎÉÖ?∑Ï? needsFirstWaterÍ∞Ä false?∏Îç∞ Ï≤?Q Î¨ºÏùÑ ?ÑÏßÅ ??Ï§Ä ?ÅÌÉú?????àÏùå.
 * wateredAtListÍ∞Ä [growthStartedAt] ??Î≤àÎøê?¥Î©¥ "Ï≤?Î¨??ÑÏöî"Î°?Î≥∏Îã§.
 */
...
function isPlantAwaitingPlayerFirstPour(plant) {
  ...
  /** ?¨Ï? ÏßÅÌõÑ Î°úÏª¨ ?ÅÌÉú??wateredAtListÍ∞Ä ÎπÑÏñ¥ ?àÏùå. stabilizeFirstWaterHintFlags??Î®∏Ï? ?úÏóêÎß??∏Ï∂ú?? */
  if (wl.length === 0) return true;
  if (wl.length !== 1) return false;
  return Math.abs(Number(wl[0]) - gs) < 20000;
}

/** 4¬∑5?????????Î¨ºÎ∞©??UI ??(???????Ï§???Í≤πÏ≥ê Î≥¥Ïù¥??Î¨∏Ï†ú) */
...
/** ÎßàÎ•∏ ??????????????ÑÍ? ????Ï£ºÍ∏∞¬∑?Îß?ÏßÅÌõÑ ?Íπ?????Í≤ÉÏ≤ò??Î≥¥Ïùº ?????*/
...
/** 4???Ä ?ùÏ°¥ ??Í∞ÄÎ£??ÜÏù¥ level5GrowMs ???êÎèô 5??(Í∞ÄÎ£®Îäî 3??Îß? */
...
/** Íµ¨ÏÑ∏?Î∏? Í∞Ä?4?? ?Í±???ÏßÑÌñâ Ï§ëÏù¥????5????Î®??? */
...
/** ?åÎ†à?¥Ïñ¥ Í∑ºÏ≤ò(Î¨ºÏ£ºÍ∏?Í±∞Î¶¨) ?ùÎ¨º ???¨Ïù∏???∏Î≤ÑÍ∞Ä ?ÜÏùÑ ??Í∞ôÏ? ?∏Î≤Ñ ?®Í≥º */
...
/** ?¨Ïù∏???ÑÎûò ?ùÎ¨º Ï§?Í∞Ä?§ÏßÑ(?§¬∑Ïûë?Ä Y) ?ùÎ¨º ?∞ÏÑ† ??Í≤πÏπ® ?????ùÎ¨º???ÑÎãå ???ùÎ¨º ?∏Î≤Ñ */
...
/** Í≥†Ï†ï UI(?§Ï†ï¬∑Ï±ÑÌåÖ¬∑?òÌä∏¬∑?¨Ìçº?? ??Î∏åÎùº?∞Ï? title ?Ä??#plant-hover-label ?§Ì??ºÎ°ú ?®Ï∂ï???úÏãú */
function pickUiShortcutHoverTarget(clientX, clientY) {
  ...
  if (settingsButton && over(settingsButton)) {
    return { anchorEl: settingsButton, text: "?§Ï†ï: Esc" };
  }
  if (!worldSocialUiReady) return null;
  if (worldChatToggleBtn && over(worldChatToggleBtn)) {
    return { anchorEl: worldChatToggleBtn, text: "Ï±ÑÌåÖ: C" };
  }
  if (worldHeartBtn && over(worldHeartBtn)) {
    return { anchorEl: worldHeartBtn, text: "?òÌä∏: H" };
  }
  if (worldSadBtn && over(worldSadBtn)) {
    return { anchorEl: worldSadBtn, text: "?¨Ìçº?? Ctrl+S" };
  }
  return null;
}

/** `.world`??transform???àÏúºÎ©?Í∑??àÏùò fixed??Î∑∞Ìè¨?∏Í? ?ÑÎãà???îÎìú Í∏∞Ï????òÏñ¥ Ï¢åÌëúÍ∞Ä ?¥Í∏ã????UI ?åÌä∏Îß?bodyÎ°?*/
...
/** 4¬∑5?®Í≥Ñ ?Ä PNG???¨Ï? ??Î≥ÑÎèÑ spot)??Í≤πÏ≥ê Î≥¥Ïù¥Î©?spot ?àÏù¥?¥Î? ?àÎã§. */
...
/**
 * ?§ÌîÑ?ºÏù¥??Î∞úÎ∞ë¬∑?úÍ∞Å Ï§ëÏã¨??plant spot Ï§ëÏïô¬∑?ÖÏóê ÎßûÏ∂§.
 * 4¬∑5?®Ï? PNG bbox ?µÏª§(MATURE_SPRITE_ANCHORS)Î°?Î≥¥Ï†ï.
 */
...
/** ?∞Ïñ¥ 4?? ?êÎèô ?±Ïû•(Í∞ÄÎ£??ÜÏùå) Íµ¨Í∞Ñ??Ï¥àÎ°ù Í≤åÏù¥ÏßÄ ÎπÑÏú®; ?¥Îãπ ?ÜÏúºÎ©?null */
...
/** ????????????Í≥?ÏßÑÌñâ¬∑Í∞Ä?????????Ï¥àÎ°ù Í≤åÏù¥ÏßÄÍ∞Ä Ï±ÑÏõåÏßÄ???*/
...
/** ÍΩÉ¬∑ÎÇòÎ¨¥¬∑ÏÑ†?∏Ïû• 5??ÏµúÏ¢Ö) ???àÏãπ 3?®Ï≤ò??Î¨º¬∑ÏàòÎ∂?Í≤åÏù¥ÏßÄ ?ÜÏùå */
...
/** 3?®Í≥Ñ ?àÏãπ¬∑4¬∑5?±Ïàô ???±Ïû•??Î©àÏ∂ò ?ùÎ¨º ???∏Î≤Ñ Îß?Í∞ïÏ°∞ ?ùÎûµ(UI ?®ÎÑê?Ä ?†Ï?) */
...
function getPlantSoilSrc(plant) {
  if (plant.status === "rotten" || plant.isOverwatered) return "?¥Î?ÏßÄ/soil-rotten.png";
  if (plant.status === "wet") return "?¥Î?ÏßÄ/soil-wet.png";
  if (plant.status === "dry") return "?¥Î?ÏßÄ/soil-dry.png";
  return "?¥Î?ÏßÄ/tilled-soil.png";
}
...
function ensureExtraPlantElements(plant) {
  ...
  spotElement.src = "?¥Î?ÏßÄ/tilled-soil.png";
  ...
  sproutElement.src = "?¥Î?ÏßÄ/sprout.png";
  ...
  waterNeededElement.src = "?¥Î?ÏßÄ/water-needed.png";
...
/** Í≥µÏú† ?§ÎÉÖ?? ???®Ïïó ?∏ÎìúÎß??úÍ±∞(?ùÎ¨º/Í≤åÏù¥ÏßÄ???†Ï?¬∑?¨Ïó∞Í≤∞Î°ú ÍπúÎπ°??Î∞©Ï?) */
...
function updateBucketPosition() {
  bucket.src = isBucketFull ? "?¥Î?ÏßÄ/bucket-full.png" : "?¥Î?ÏßÄ/bucket-empty.png";
  playerBucketOverlay.style.backgroundImage =
    'url("' + (isBucketFull ? "?¥Î?ÏßÄ/bucket-full.png" : "?¥Î?ÏßÄ/bucket-empty.png") + '")';
  ...
  if (heldItem === HELD_ITEM_BUCKET) {
    ...
    if (isHoldingMainBucket() || isHoldingExtraBucket()) {
      ...
      bucket.src = mainGround.isFull ? "?¥Î?ÏßÄ/bucket-full.png" : "?¥Î?ÏßÄ/bucket-empty.png";
      bucket.style.display = "block";
...
/** ?ùÎ¨º???¨Ïù∏¬∑?ÅÏù∏¬∑?∞Í∏à?†ÏÇ¨ ?Ä??Ï§??¥Îèô¬∑?ÅÌò∏?ëÏö© ??Í≤åÏûÑ?åÎ†à??Î∂àÍ?. Í±∞Îûò¬∑?úÏûë UI???úÏô∏. */
...
/** Í≥ºÏÑ±?? Îπ????∞Ïñ¥0¬∑?àÏãπ ?ÜÏùå) Íµ¨Í∞Ñ?Ä ?ºÎ∞ò ?®ÏïóÍ≥??ôÏùº?òÍ≤å ?†Ï?, ?¥ÌõÑ?êÎßå 3?®Í≥ÑÎ°??ïÏ†ï */
...
/** Î∏åÎùº?∞Ï? Í∏∞Î≥∏ title(ÏßÄ?? ?Ä??CSS data-ovc-tip?ºÎ°ú Î∞îÎ°ú ?®Îäî ?§Î™Ö */
...
/** ?¥ÏïÑ ?àÎäî ?ùÎ¨º¬∑ÎßàÎ•∏/?©Ï? ?? ?Ö¬∑ÏÉà?π¬∑Ì????åÏú†?êÎ≥ÑÎ°?Í∞ÅÍ∞Å ?∞Î°ú Î≤àÌò∏ Î∂Ä??*/
...
/** Í∏âÏàò Í∞Ä?•ÌïòÏßÄÎß?Î¨ºÏ£ºÍ∏?Í±∞Î¶¨ Î∞ñÏù∏ Í∞Ä??Í∞ÄÍπåÏö¥ ?ùÎ¨ºÍπåÏ? Í±∞Î¶¨(?ÜÏúºÎ©?Infinity) */
...
/** ?òÎ∂Ñ 0Ïπ?ÎßàÎ¶Ñ Ïπ¥Ïö¥?∏Îã§?? ???∏Î≤Ñ ???åÎ? ?ÑÏä§ Í∞ïÏ°∞ */
...
function isPlantSpotOverlappingTreeNoPlantZone(plantX, plantY) {
  ...
  /** Ï§ÑÍ∏∞¬∑ÎøåÎ¶¨Í∞Ä Í∑∏Î†§Ïß??íÏù¥Îß?ÎßâÏùå(?¥Ï†Ñ??trunkBottom??ÏßÄÎ©??ùÍπåÏßÄ Í∞Ä??Í∞ôÏ? x???ÑÏ≤¥ y?êÏÑú ?¨Í∏∞ Î∂àÍ??Ä?? */
...
/**
 * growthTierÎß??∞Î©¥ ?àÏãπ??Ïª§ÎèÑ ?§Îû´?ôÏïà 0?¥Îùº Î∞òÍ≤Ω?????ìÏñ¥Ïß????úÍ∞Å ?®Í≥Ñ(getSproutStage)?Ä Í∞ôÏù¥ ?Ä.
 * ?úÍ±∞???ùÎ¨º?Ä ?ÑÎ≥¥?êÏÑú ÎπºÍ≥†, maturity??0??Î°?Ï∫?
 */
...
function useBucket() {
  ...
  refillWellIfNeeded();
  // ???ÖÎ†•?Ä Î≥¥ÌÜµ ?§Ïùå updateBucketPositionÎ≥¥Îã§ Î®ºÏ? ??????Ï¢åÌëúÍ∞Ä ?åÎ†à?¥Ïñ¥ ???ÑÎ†à????úºÎ©?
  if (heldItem === HELD_ITEM_BUCKET) {
    const bucketSize = getBucketSize();
    const handPosition = getHandPosition(bucketSize.width, bucketSize.height);
    bucketX = handPosition.x;
  ...
  // Í∏âÏàò Í∞Ä?•Ìïú ?ùÎ¨º???àÏúºÎ©?Q????ÉÅ ?ùÎ¨º??Î∂ìÍ∏∞(?∞Î¨ºÍ≥?Í±∞Î¶¨ Í≤πÏπ® ???∞Î¨ºÎ°?Îπ†Ï???Î¨∏Ï†ú Î∞©Ï?).
  if (
    nearWellPour &&
    wellState.water < maxWellWater &&
    !wateringTarget
  ...
  // Ï∞??ëÎèô?¥Ïù∏??Í∏âÏàò???ùÎ¨º???ÜÍ≥† ?∞Î¨º??Í∞Ä?? ?òÎ∂ìÍ∏?Î∂àÍ? ?àÎÇ¥ + ?§Ìîå?òÏãúÎ°??ºÎìúÎ∞?
...
/** ?åÏÉâ ÎßàÎ≤ï Í∞ÄÎ£??¨Ïö© Í∞Ä??Í±∞Î¶¨¬∑?òÎüâ¬∑ÏßÑÌñâ Ï§??¨Î?) */
...
function updatePlantState() {
  ...
  if (
    ...
    // ÎßàÎ•∏ ??ÎßêÎùº Ï£ΩÏ? ?ëÎ¨º): ??ÎπÑÏ? ¬∑ Î¨?Ï£ºÍ∏∞ Î∂àÍ? ¬∑ plantDrySoilClearMs ??Ïπ??úÍ±∞
    plantRuntime.status = "dry";
    plantRuntime.isOverwatered = false;
  ...
  if (mainSoilRotten) {
    plantSpot.src = "?¥Î?ÏßÄ/soil-rotten.png";
  } else if (plantRuntime.status === "wet") {
    plantSpot.src = "?¥Î?ÏßÄ/soil-wet.png";
  } else if (plantRuntime.status === "dry") {
    plantSpot.src = "?¥Î?ÏßÄ/soil-dry.png";
  } else {
    plantSpot.src = "?¥Î?ÏßÄ/tilled-soil.png";
  }
...
function updateWellImage() {
  well.src = wellState.water > 0 ? "?¥Î?ÏßÄ/well.png" : "?¥Î?ÏßÄ/well-empty.png";
}

function updateWellCard() {
  const isVisible = isNearWellForCard();
  const waterRatio = wellState.water / maxWellWater;
  const wellImage = wellState.water > 0 ? "?¥Î?ÏßÄ/well.png" : "?¥Î?ÏßÄ/well-empty.png";
  ...
  return fetch(url, {
    ...
    return response.json().then(function (data) {
      if (!response.ok || !data.ok) {
        throw new Error(data.message || "?îÏ≤≠???§Ìå®?àÏäµ?àÎã§.");
      }
...
function buildCharacterColorGrid() {
  ...
  characterColors.forEach(function (color) {
    ...
    button.setAttribute("aria-label", color + " ?âÍπî");
...
function syncLocalPlayerVisibility() {
  ...
  if (hasSpawnedCharacter) {
    ...
    if (!player.getAttribute("src")) {
      player.src = "?¥Î?ÏßÄ/player-white.png";
    }
    return;
  }
...
/**
 * "?çÍ∏∏??" ??"?çÍ∏∏?? ?¨Îûå??" ?ïÌÉúÎ°??òÏã†???ëÎëêÎ•??ÑÏ†Å. `?ÑÏ≤¥:` Í∞Ä ?àÏúºÎ©??úÍ±∞ ??Í∑ìÎßê ?ëÎëêÎ°??ÑÌôò.
 */
...
function setupMultiplayer() {
  ...
  if (isSharedWorldSyncPausedForTutorial()) {
    ...
    updateMultiplayerStatus(
      "?úÌÜ†Î¶¨Ïñº: ?§Î•∏ ?åÎ†à?¥Ïñ¥/?∏ÏÉÅ ÎπÑÍ≥µ??¬∑ Î©Ä??ÎØ∏Ïó∞Í≤?
    );
...
function sendMultiplayerPresence(forceSend) {
  ...
  if (
    ...
    Promise.resolve(multiplayerChannel.send({
      ...
      addNetworkDebugLog(
        "broadcast error: " + (error && error.message ? error.message : "?®Îùº???úÎ≤Ñ ?ïÏù∏ ?ÑÏöî")
      );
    });
    lastBroadcastAt = now;
...
function handleRemoteBucketBroadcast(payload) {
  ...
  // Î©îÏù∏ ?ëÎèô?¥Î? ?§Í≥† ?àÏùÑ ?åÎßå Î°úÏª¨ ?ÅÌÉúÎ•??†Ï? ??Ï∂îÍ? ?ëÎèô?¥Îßå ??Í≤ΩÏö∞ ?êÍ≤© Î©îÏù∏ ?ôÍ∏∞???àÏö©.
  if (isHoldingMainBucket()) {
    return;
  }
...
function syncPresenceToDatabase(state) {
  ...
  window.OVCOnline.savePresence(state).catch(function (error) {
    addNetworkDebugLog(
      "presence db save error: " + (error && error.message ? error.message : "?®Îùº???úÎ≤Ñ ?ïÏù∏ ?ÑÏöî")
    );
  }).finally(function () {
...
function pollPresenceDatabase() {
  ...
  window.OVCOnline.listPresence(
    ...
    (players || []).forEach(function (state) {
      ...
      // DB ?¥ÎßÅ ?úÍ∞Å(now)???ÑÎãà??presence.updatedAt???®Ïïº Í≥†ÏïÑ ?∏ÏÖò??
      // ?òÎπÑ Í∂åÌïú ?†Í±∞??Í≥ÑÏÜç ?ºÏñ¥?§Ï? ?äÏùå(Îß??¥Îßà??nowÎ°?Í∞±Ïã†?òÎ©¥ ?ÅÏõê??"?¥ÏïÑ ?àÏùå").
    ...
    addNetworkDebugLog(
      "presence db poll error: " + (error && error.message ? error.message : "?®Îùº???úÎ≤Ñ ?ïÏù∏ ?ÑÏöî")
    );
  }).finally(function () {
...
function updateMultiplayerStatus(statusText) {
  ...
  const statusLabel =
    multiplayerStatusText === "?∞Í≤∞?? ||
    multiplayerStatusText === "?∞Í≤∞Ï§? ||
    multiplayerStatusText === "Ï∫êÎ¶≠???†ÌÉù ?? ||
    multiplayerStatusText === "Ï¥àÍ∏∞??Ï§? ||
    (typeof multiplayerStatusText === "string" &&
      multiplayerStatusText.indexOf("?úÌÜ†Î¶¨Ïñº") !== -1)
      ? multiplayerStatusText
      : "?∞Í≤∞ ?àÎê®";
  multiplayerStatus.textContent =
    "Î©Ä??" + statusLabel + " / Î°úÍ∑∏??" + getOnlinePlayerCount();
}
...
function syncPlayerColorToServer(forceSync) {
  ...
  if (window.OVCOnline && window.OVCOnline.isConfigured()) {
    window.OVCOnline.savePlayerColor(currentUserId, colorToSync).then(function () {
      ...
      "Î°úÏª¨ ?Ä???§Ìå®: " +
        (error && error.message ? error.message : "?®Îùº???úÎ≤Ñ ?ïÏù∏ ?ÑÏöî")
      );
    });
    return;
  }
    ...
    if (typeof window.OVCOnline.validateSession === "function") {
      ...
      if (!storedToken) {
        ...
        if (typeof window.OVCOnline.getAccount === "function") {
          const account = await window.OVCOnline.getAccount(currentUserId);
          if (!account) {
            showOnlineDebugMessage("??†ú??Í≥ÑÏ†ï?ÖÎãà?? Î°úÍ∑∏?ÑÏõÉ?©Îãà??");
      ...
      if (!isValid) {
        showOnlineDebugMessage("?§Î•∏ Í∏∞Í∏∞?êÏÑú Î°úÍ∑∏?∏Îêò??Î°úÍ∑∏?ÑÏõÉ?©Îãà??");
...
/** ?®Îùº?? Í∂åÌïú ??ßå ?úÎ?. Í∂åÌïú ?§ÎÉÖ?∑Ïù¥ ?äÍ∏∞Î©?Î°úÏª¨ ?¥Î∞±(Í≥†ÏïÑ presence¬∑Í∂åÌïú ?†Ïã§ ?ÄÎπ?. */
...
/*
  ...
  if (!waypoint || now >= waypoint.endAt) {
    ...
    // Pixels-per-frame to ms @ ~60fps. ÏßßÏ? ?§Î¶¨?êÎèÑ 2.4sÎ•?Í∞ïÏ†ú?òÎ©¥ Í±∞Ïùò ?úÏûêÎ¶¨Ïóê Î©àÏ∂ò Í≤ÉÏ≤ò??Î≥¥ÏûÑ ??Í±∞Î¶¨ ÎπÑÎ? + ??? Î∞îÎã•.
...
function authoritySpawnButterfliesIfNeeded(now) {
  ...
  if (!butterflyState.lastSpawnAt) {
    // ÏµúÏ¥à ????Ïß??Îπ?Ï±ÑÏö¥ ??????Ï¶âÏãú capÍπ? Ï±?. ?? ??úÍ? ?????
    // lastSpawnAtÎß?0?¥Î©¥(?§ÎÉÖ???ÑÎùΩ¬∑Î≥ëÌï©) ?¨Í∏∞??5ÎßàÎ¶¨Î•??úÍ∫ºÎ≤àÏóê ?£Ïñ¥ Î¶¨Ïä§??Ï£ºÍ∏∞Í∞Ä Î¨¥Ïãú??
...
function updateButterflies() {
  ...
  // ?òÎπÑ ?ÑÏπò ?úÎ??Ä Í∂åÌïú ?¥Îùº?¥Ïñ∏??Í∞Ä????? sessionId)Îß??òÌñâ. ÎπÑÍ∂å????ù¥
  // Í∞ôÏù¥ ?Î¶?????Ï¢åÌëú? ????2Ï∞Ω¬???????????????
  ...
  // ÎπÑÍ∂å?? ?§ÎÉÖ?∑Ïùò butterfly.x/y(Í∂åÌïú???£Ï? ?ÑÏπò+?åÎü¨??Îß?Î∂Ä?úÎüΩÍ≤??∞ÎùºÍ∞?
  const smoothRemoteButterflies =
    sharedHydrated && onlineAvailable && !isButterflyAuthority();
  /** ????Í∞ÑÍ≤©??ÎßûÏ∂ò ÏßÄ??Î≥¥Í∞Ñ(Ïß? dt????Í≥°ÏÑ†???Í∏∞Ï? ?Í≤?. */
  ...
  /** Í≥ºÎèÑ???Î∞????ÎßâÏùå(?????? Ï∫? Í≥ÑÎã®Ï≤òÎüº Î≥¥Ïù¥???). */
  ...
  // Render (?°Í∏∞ ?êÏ†ï?Ä Î≥¥Í∞Ñ??Ï§ëÏã¨ `_catchProbe*`?Ä ?ôÏùº ??ÎπÑÍ∂å????óê???îÎ©¥Í≥??ºÏπò)
  const aliveIds = {};
  ...
  butterflyState.list.forEach(function (butterfly) {
    ...
    if (smoothRemoteButterflies) {
      ...
      // Í∂åÌïú ?????¬∑????? ?? Ï¢åÌëúÍ∞Ä ?ÏßÑÏã§? ?Í∏???????Î≥¥Í∞Ñ?Î©?
      // ?????Í∞±Ïã†??????¬∑Í≤ΩÎ°úÎ≥???Î©???Î∞ïÏûê ????Î≤ÑÎ≤Ö?Ï≤??Î≥¥ÏûÑ(?Ï∞?Í∂åÌïú ?????.
