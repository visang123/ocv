...
controlsOverlay.innerHTML =
  ...
  '<div class="controls-header"><strong>Ï°∞ÏûëÎ≤?/strong><button id="controls-close-button" type="button" aria-label="?´Í∏∞">√ó</button></div>' +
...
function setupMultiplayer() {
  if (!hasSpawnedCharacter) {
    updateMultiplayerStatus("Ï∫êÎ¶≠???†ÌÉù ??);
...
function syncPlayerColorToServer(forceSync) {
  ...
  postJson("/api/player/color", {
    ...
    showOnlineDebugMessage(
      "Î°úÏª¨ ?Ä???§Ìå®: " +
      (error && error.message ? error.message : "Î°úÏª¨ ?úÎ≤Ñ ?ïÏù∏ ?ÑÏöî")
    );
  });
}
...
async function loadAdminAccounts() {
  adminMessage.textContent = "Í≥ÑÏ†ï Î∂àÎü¨?§Îäî Ï§?..";
  ...
  try {
    ...
    adminMessage.textContent = accounts.length + "Í∞?Í≥ÑÏ†ï";
  } catch (error) {
...
function renderAdminAccounts(accounts) {
  ...
  if (!accounts.length) {
    ...
    empty.textContent = "Í∞Ä?ÖÎêú Í≥ÑÏ†ï???ÜÏäµ?àÎã§.";
  ...
  accounts.forEach(function (account) {
    ...
    name.textContent = account.name || "?¥Î¶Ñ ?ÜÏùå";
    meta.textContent =
      (account.color || "???ÜÏùå") +
      " / " +
      formatAdminDate(account.created_at);
    deleteButton.textContent = "??†ú";
    deleteButton.type = "button";

    deleteButton.addEventListener("click", async function () {
      if (!confirm((account.name || "??Í≥ÑÏ†ï") + "????†ú?†Íπå??")) return;
      ...
      try {
        ...
        adminMessage.textContent = "??†ú ?ÑÎ£å";
        loadAdminAccounts();
      } catch (error) {
...
function formatAdminDate(value) {
  if (!value) return "?†Ïßú ?ÜÏùå";

  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "?†Ïßú ?ÜÏùå";
...
async function validateCurrentAccount() {
  ...
  try {
    if (window.OVCOnline.isConfigured() && typeof window.OVCOnline.validateSession === "function") {
      ...
      if (!isValid) {
        showOnlineDebugMessage("?§Î•∏ Í∏∞Í∏∞?êÏÑú Î°úÍ∑∏?∏Îêò???êÎèô Î°úÍ∑∏?ÑÏõÉ?©Îãà??");
